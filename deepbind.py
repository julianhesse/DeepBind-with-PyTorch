...
torch.save(best_hyperparameters, out_folder + 'best_hyperpamarameters.pth')
...
best_hyperparameters = torch.load(out_folder + 'best_hyperpamarameters.pth')
...
for number_models in range(6):
  ...
  #
      ...
      if AUC_training>best_AUC:
          ...
          torch.save(state, out_folder + 'MyModel_2.pth')


checkpoint = torch.load(out_folder + 'MyModel_2.pth')
...
with open(out_folder + 'prediction.out', 'w') as f:
    for pred in preds:
        for p in pred:
            f.write(str(p) + '\n')
